...
class ReactorSystems:
    ...
    def update_A(self):
        ...
        # Check if there is enough coolant to allow flow
        if self.volA_1 < 0.1 * self.VOLUME_A_1:  # More lenient threshold
            self.flowA *= 0.5  # Reduce flow gradually
...
data_pin_595 = Pin(13, Pin.OUT)   # DS or SER (14 on IC)
latch_pin_595 = Pin(14, Pin.OUT)  # STCP (12 on IC)
clock_pin_595 = Pin(12, Pin.OUT)  # SHCP (11 on IC)
pins_595 = 8 * [1]  # Store states of output pins, initially high

# 74HC165 input shift register
load_pin_165 = Pin(33, Pin.OUT)   # SH/LD (1 on IC)
clock_pin_165 = Pin(27, Pin.OUT)  # CLK (2 on IC)
data_pin_165 = Pin(32, Pin.IN)    # DS or SER (10 on IC)
pins_165 = 8 * [0]  # Store states of input pins
...
def read_74hc165():
    """Read 8-bit input from 74HC165 and return a list of pin states [QA, QB, ..., QH]."""
    load_pin_165.off()   # Load parallel inputs (active LOW)
    sleep_ms(1)
    load_pin_165.on()    # Shift mode
    
    pin_states = pins_165  # Initialize list for QA-QH
    for i in range(8):
        clock_pin_165.off()
        sleep_us(5)
        pin_states[7 - i] = data_pin_165.value()  # MSB first (QH is first shifted out)
        clock_pin_165.on()
        sleep_us(5)
    
    print("Raw 74HC165 read:", pin_states)  # Debug output
    
    return pin_states  # Returns [QA, QB, QC, QD, QE, QF, QG, QH]
...
def write_74hc595(pin_states):
    ...
    for i in range(8):  # From QH to QA (MSB first)
        clock_pin_595.off()
...
def handle_switch(row_index: int, col_index: int):
    ...
    if col_index == 3 and row_index == 2:
        time_step_sim = 5.0  # 5 second time step instead of 1

    if col_index == 4:
        if row_index == 0:
            reactor.is_autopilot_rods = True
        else:
            reactor.is_autopilot_rods = False
...
def scan_switches():
    ...
    """
    for col_idx in range(5):  # 5 columns (0-4)
        # Activate current column (set LOW)
        pins_595 = [1] * 8  # Default all HIGH
        pins_595[col_idx] = 0  # Set current column LOW
        write_74hc595(pins_595)
        
        # Read rows
        row_states = read_74hc165()
        
        # Check each row (0-4)
        for row_idx in range(5):
            if row_states[row_idx] == 0:  # Active LOW
                handle_switch(row_idx, col_idx)
        
        # Deactivate column (set HIGH)
        pins_595[col_idx] = 1
        write_74hc595(pins_595)
        sleep_ms(1)
...
#         last_refreshed += time_elapsed
    ...
    time_elapsed = time() - time_start
    time_now += time_elapsed  # Handle time boost
    last_refreshed += time_elapsed
    time_start = time()
