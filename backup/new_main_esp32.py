...
# Button matrix pins
cols = [Pin(14, Pin.OUT), Pin(15, Pin.OUT), Pin(16, Pin.OUT), Pin(17, Pin.OUT), Pin(21, Pin.OUT)]
...
def handle_switch(row_index: int, col_index: int):
    ...
    if col_index == 4:
        if row_index == 0:
            self.is_autopilot_rods = True
        else:
            self.is_autopilot_rods = False
...
def scan_switches(cols: list, rows: list):
    ...
    for col_idx, col in enumerate(cols):
        ...
        for row_idx, row in enumerate(rows):
            if row.value() == 0:  # Switch activated (active low)
